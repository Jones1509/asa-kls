          ...
          <div key={group.date}>
            ...
            <div className="space-y-3">
              {group.messages.map((m: any, i: number) => {
                ...
                const showName = !isSelf && (!prevMsg || prevMsg.user_id !== m.user_id);
                const initials = ((m.profiles as any)?.full_name || "?").split(" ").map((n: string) => n[0]).join("").slice(0, 2);
                ...
                return (
                  ...
                  >
                    {!isSelf && showName && (
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-[10px] font-bold text-muted-foreground flex-shrink-0 mt-1">
                        {initials}
                      </div>
                    )}
                    {!isSelf && !showName && <div className="w-8 flex-shrink-0" />}
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                      ...
                      {showName && <p className="text-[11px] font-semibold mb-0.5 opacity-70">{(m.profiles as any)?.full_name || "Ukendt"}</p>}
